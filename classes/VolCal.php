...
class VolCal {
    ...
    // There are two timeframes, the start and end of a single event, and the Date start and end of the repeated events
        ...
        $g[date("Y-m-d", strtotime($evt->get_start()))] = array("start" => date("Y-m-d H:i:s", strtotime($evt->get_start())), "end" => date("Y-m-d H:i:s", strtotime($evt->get_end())));
        ...
        $maxRepeatEvents = $uS->maxRepeatEvents;
    ...
    public static function emailAdmin(cEventClass $evt, $numEvents) {
        ...
        $email->set_subject("Shift Cancellation Notice for " . $evt->get_volDescription());
