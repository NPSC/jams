...
*
* ©2013 on Profit Software Corporation - all rights reserved.
*
* Disclaimer:
...
abstract class MercRequest {
     ...
     */
    public function submit(array $gway) {
        ...
        try {
            ...
            $txClient = new SoapClient($gway['Credit_Url']);
...
class InitCiRequest extends MercRequest {
    ...
    function __construct($pageTitle = '', $displayStyle = '') {
    ...
    protected function execute(\SoapClient $txClient, array $data) {
        return new InitCiResponse($txClient->InitializeCardInfo($data), $this->gateWay['CardInfo_Url']);
    }
...
class InitCkOutRequest extends MercRequest {
    ...
    protected function execute(\SoapClient $txClient, array $data) {
            return new InitCkOutResponse($txClient->InitializePayment($data), $this->gateWay['Checkout_Url']);
        }
...
abstract class MercTokenRequest extends MercRequest {
    ...
    public function submit(array $gway) {
        ...
        try {
            $txClient = new SoapClient($gway['Trans_Url']);
            $xaction = $this->execute($txClient, $data);
...
class CreditSaleTokenRequest extends MercTokenRequest {
    ...
    protected function execute(\SoapClient $txClient, array $data) {
        return new CreditTokenResponse($txClient->CreditSaleToken($data), 'CreditSaleTokenResult');
    }
...
class CreditReturnTokenRequest extends MercTokenRequest {
    ...
    protected function execute(\SoapClient $txClient, array $data) {
        return new CreditTokenResponse($txClient->CreditReturnToken($data), 'CreditReturnTokenResult');
    }
...
class CreditTokenResponse extends MercResponse {
     ...
     * @throws Hk_Exception_Payment
     */
    function __construct($response, $resultName) {
        parent::__construct($response);
