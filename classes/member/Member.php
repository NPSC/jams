...
abstract class Member {
     ...
     * @param PDO $dbh
     * @param int $nid
     * @param MemDesignation $defaultDesignation
    ...
    public static function GetDesignatedMember(PDO $dbh, $nid, $defaultMemberBasis) {
        ...
        $nRS = Member::loadNameRS($dbh, $nid);

        if ($nid > 0) {
            // Existing member
            if (isset($uS->nameLookups[GL_TableNames::MemberBasis][$nRS->Member_Type->getStoredVal()])) {

                $desig = $uS->nameLookups[GL_TableNames::MemberBasis][$nRS->Member_Type->getStoredVal()][Member::SUBT];

            } else {
                throw new Hk_Exception_Runtime("This member has an Undefined Member Basis: " . $nRS->Member_Type->getStoredVal() . ".");
            }

        } else {
            // New member
            if (isset($uS->nameLookups[GL_TableNames::MemberBasis][$defaultMemberBasis])) {

                $desig = $uS->nameLookups[GL_TableNames::MemberBasis][$defaultMemberBasis][Member::SUBT];

            } else {
                // defaulut member type is undefined
                throw new Hk_Exception_InvalidArguement("Supplied default Member Basis is undefined: " . $defaultMemberBasis . ".");
            }
        }
    ...
    protected static function loadNameRS(PDO $dbh, $nid) {
        ...
        if ($nid > 0) {
            ...
            if (count($rows) == 1) {
                ...
                throw new Hk_Exception_Runtime("There is no record for Member Id = $nid");
            }
        }
        return $nRS;
    }
    ...
    public abstract function createDemographicsPanel();
    ...
    public abstract function createMiscTabsMarkup();
    ...
    public function saveChanges(PDO $dbh, array $post) {
        ...
        if ($n->idName->getStoredVal() == 0) {
            ...
            if (isset($uS->nameLookups[GL_TableNames::MemberBasis][$n->Member_Type->getNewVal()])) {

                if ($uS->nameLookups[GL_TableNames::MemberBasis][$n->Member_Type->getNewVal()][Member::SUBT] == MemDesignation::Individual) {
                    $n->Record_Member->setNewVal(1);
                } else if ($uS->nameLookups[GL_TableNames::MemberBasis][$n->Member_Type->getNewVal()][Member::SUBT] == MemDesignation::Organization) {
                    $n->Record_Company->setNewVal(1);
                } else {
                    throw new Hk_Exception_UnexpectedValue("Member Designation not determined.");
                }
            } else {
                throw new Hk_Exception_UnexpectedValue("No DB match for Member Basis.");
            }

            // Verify Member Status
            if (isset($uS->nameLookups[GL_TableNames::MemberStatus][$n->Member_Status->getNewVal()]) === FALSE) {
                throw new Hk_Exception_UnexpectedValue("No DB match for Member Status.");
            }
            ...
            if (is_null($n->Member_Type->getNewVal()) === FALSE && $n->Member_Type->getNewVal() !=  $n->Member_Type->getStoredVal()) {
                ...
                if ($uS->nameLookups[GL_TableNames::MemberBasis][$n->Member_Type->getNewVal()][Member::SUBT] !=
                        $uS->nameLookups[GL_TableNames::MemberBasis][$n->Member_Type->getStoredVal()][Member::SUBT]) {

                    throw new Hk_Exception_Runtime("Cannot change member basis");
                }
            }
            ...
            if (is_null($n->Member_Status->getNewVal()) === FALSE && $n->Member_Status->getNewVal() != $n->Member_Status->getStoredVal()) {
                ...
                switch ($n->Member_Status->getNewVal()) {
                    ...
                    case MemStatus::Duplicate:
                        throw new Hk_Exception_Runtime("Cannot directly change member status to Duplicate.");
                        break;
                    ...
                    case MemStatus::ToBeDeleted:
                        ...
                        if ($result != '') {
                            throw new Hk_Exception_Runtime("Cannot Delete this member: " . $result);
                        }
                        break;
        ...
        // Process member
        $message = $this->processMember($post);
    ...
    protected abstract function processMember(array $post);
    ...
    protected function processCommon(array $post) {
        ...
        if (isset($post[$idPrefix.'rbPrefMail'])) {
            foreach ($uS->nameLookups[GL_TableNames::AddrPurpose] as $code) {
                if ($code[Member::CODE] == $post[$idPrefix.'rbPrefMail']) {
        ...
        if (isset($post[$idPrefix.'rbEmPref'])) {
            foreach ($uS->nameLookups[GL_TableNames::EmailPurpose] as $code) {
                if ($code[Member::CODE] == $post[$idPrefix.'rbEmPref']) {
        ...
        if (isset($post[$idPrefix.'rbPhPref'])) {
            foreach ($uS->nameLookups[GL_TableNames::PhonePurpose] as $code) {
                if ($code[Member::CODE] == $post[$idPrefix.'rbPhPref']) {
    ...
    public function retireAllVolunteer(PDO $dbh, $user) {
        ...
        $query = "update name_volunteer2 set Vol_Status = 'i', Vol_End = now(), Updated_By=:byuser, Last_Updated=now()
            where idName = :id and Vol_Status <> 'i';";
        $stmt = $dbh->prepare($query);
