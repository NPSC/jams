...
function recordDonation(PDO $dbh, $maxDonationAmt, $id, $parms) {
    ...
    if (isset($data["dselStudent"]) && $campaign->get_type() == CampaignType::Scholarship) {
        $studentId = filter_var($data["dselStudent"], FILTER_SANITIZE_NUMBER_INT);
        $fundCode = $studentId;
    }
    ...
    if ($acId > 0) {
        ...
        if ($donId > 0) {
            ...
            if ($studentId >= 0) {
                ...
                $ssg->makeDonation($dbh, $amt, $donId, $id, $uS->username);
                $ssg->save($dbh, $uS->username);
            }
...
function deleteDonation(PDO $dbh, $donId, $uname) {
    ...
    if ($donId > 0) {
        ...
        $stmt = $dbh->prepare("update donations set Status='d', Last_Updated=now(), Updated_By=:ub where iddonations = :did;");
        $stmt->execute(array(':ub'=>$uname, ':did'=>$donId));

        if ($stmt->rowCount() == 1) {

            // Mark any scholarships as deleted.
            $stmt = $dbh->prepare("update scholarship set Is_Deleted = 1, Last_Updated=now(), Updated_By=:ub where idDonation = :did");
            $stmt->execute(array(':ub'=>$uname, ':did'=>$donId));
