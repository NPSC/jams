...
function roomRevReport(PDO $dbh, $rbsel, $yr, $fyMonthsAdjust) {
    ...
    if ($rbsel == "cy") {
        ...
        $query = "select year(d.Date_Entered) as fy, ifnull(c.Title, 'Sub Total'), ";
    } else {
        ...
        $query = "select year(DATE_ADD(d.Date_Entered, INTERVAL $fyMonthsAdjust MONTH)) as fy, ifnull(c.Title, 'Sub Total'), ";
    }

    $query .= " sum(d.Authorized_Amount) as `Amount`
            from fees d join resource c on LOWER(TRIM(d.Product_Code)) = LOWER(TRIM(c.idResource)) and d.Status = 'c'
            where c.Type in ('room', 'rmtroom') $whClause group by fy, c.Title with rollup;";
    ...
    if ($rbsel == "cy") {
        $txtreport = "<table><tr><td colspan='3' style='text-align:center;'><em>Room Revenue by Calender Year</em></td><td> Date: " . date("M j, Y") . "</td></tr>";
        $txtreport .= "<tr><td></td><td><em>Fetched " . count($rows) . " Records</em></td></tr>";
        $txtreport .= "<tr><th>Year</th><th>Room</th><th style='text-align:right;'>Total</th></tr>";
    } else {
        $txtreport = "<table><tr><td colspan='3' style='text-align:center;'><em>Room Revenue by Fiscal Year</em></td><td> Date: " . date("M j, Y") . "</td></tr>";
        $txtreport .= "<tr><td></td><td><em>Fetched " . count($rows) . " Records</em></td></tr>";
        $txtreport .= "<tr><th>Fiscal Year</th><th>Room</th><th style='text-align:right;'>Total</th></tr>";
    }
