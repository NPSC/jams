 ...
 */
function processCategory(PDO $dbh, &$selCtrls, selCtrl &$rankCtrl, selCtrl &$dormancyCtrl, selCtrl &$volStatusCtrl) { //, &$sortCtrl) {
    $volCat = new VolCats();
    $dlFlag = false;
    $csvFlag = false;
...
//            $wClause .= " and (date_format(curdate(), '%j') < ifnull(date_format(d.Begin_Active, '%j'), 0) or date_format(curdate(), '%j') > ifnull(date_format(d.End_Active, '%j'), 366)) ";
        ...
        if (isset($_POST["btnMlCat"]) || isset($_POST["btnMlCatDL"])) {
            $query = "Select c2.Id from (" . makeSQL($wClause, $groupBy, $totalId) . ") as c2 ";
        } else if ($csvFlag == true) {
            $query = "Select distinct c2.PreferredEmail from (" . makeSQL($wClause, $groupBy, $totalId) . ") as c2 ";
        } else {
            $query = "Select * from (" . makeSQL($wClause, $groupBy, $totalId) . ") as c2 ";
        }
...
function makeSQL($whereClause, $groupBy, $totalId) {
    $query = "
select
vm.Id AS Id,
    ...
    left join vmember_categories c ON vm.Id = c.idName
    where vm.MemberStatus = 'a' $whereClause $groupBy";
