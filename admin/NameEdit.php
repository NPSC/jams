...
if (is_null($mbasis) === FALSE && isset($uS->nameLookups[GL_TableNames::MemberBasis][$mbasis])) {
    $defBasis = $mbasis;
}
...
// Relationships
$relationsMarkup = $name->createRelationsTabs($rel, 'NameEdit.php', $student);
...
if ($id == 0) {
    ...
    if ($newStudent) {
        $label = "New Student ";
        $sty = " style='background-color:#FBFF8F;' ";
    } else {
        $label = "New Person";
    }

    $NiceName = "<span style='font-size:2em;'>$label</span>";

} else {

    if ($student->isStudent() || $newStudent) {
        $label = "Student: ";
        $sty = " style='background-color:#FBFF8F;' ";
    } else {
        $label = 'Member: ';
    ...
    $NiceName = "<span style='font-size:2em;'>" .$label. $name->getMemberName() . "</span>";
}
...
if ($student->isStudent() || $newStudent) {
    History::addToStudentHistoryList($dbh, $id);
    // Student History tab markup
    $recHistory = History::getStudentHistoryMarkup($dbh);
} else {
    // Admin History - add this ID.
    History::addToMemberHistoryList($dbh, $id);
...
if ($id != 0 && $donationsFlag && $student->isStudent() === FALSE && !$newStudent) {
    ...
    $donateMkup = Donate::createDonateMarkup(
            ...
            ($uS->StudentSG ? $student->getStudentOptions() : NULL)
            );
...
$memberData['newStudent'] = $newStudent;
...
<?php echo TOP_NAV_CSS; ?>
    ...
    <body <?php echo $sty; if ($testVersion) echo "class='testbody'"; ?>>
            <?php echo $menuMarkup; ?>
        <div id="contentDiv">
            ...
            <form action="NameEdit.php" method="post" id="form1" name="form1" >
                ...
                <input type="hidden" name="hdncmd" id="hdnid" value="<?php echo $id; ?>" />
        ...
        <script type="text/javascript" src="js/genfunc1.js"></script>
