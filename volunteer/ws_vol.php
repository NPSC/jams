        ...
        if (count($parts) == 2) {
            ...
            foreach ($stmt->fetchAll(PDO::FETCH_ASSOC) as $r) {
                ...
                if ($r['PreferredEmail'] != "") {
                    // Send email
                    if ($r['Id'] == $id) {
                        $em->set_replyTo($r['PreferredEmail'], $r['Name']);
                    }
            ...
            $emStr = $emailAddrs[0]['Email'];

            for ($j = 1; $j < count($emailAddrs); $j++)  {

                $emStr .= "," . $emailAddrs[$j]['Email'];
            ...
            if ($emStr != "") {
                ...
                $em->set_bcc($emStr);
                $em->set_from($config->getString("vol_email", "Admin_Address", ""));
                $em->set_to($config->getString("vol_email", "Admin_Address", ""));
                $em->set_subject($cSubj);
                $em->set_body($cBody);
...
function changePW(PDO $dbh, $oldPw, $newPw, $uname, $id) {
    ...
    if (UserClass::_checkLogin($dbh, $uname, $oldPw) === FALSE) {
        return array('error'=>'Bad Old Password');
    }

    if (UserClass::updateDbPassword($dbh, $id, $oldPw, $newPw, $uname) === TRUE) {
